--v2.5 - Центрированный логотип, сдвинутая таблица
...
local OFFSET = 3                 -- на сколько строк сдвинуть таблицу и сообщения вниз
...
local function drawInfo(type)
    local line = 2 + OFFSET      -- сдвиг таблицы вниз
    if type == "full" then
        gpu.fill(1, 1, w, h, " ")
    end
...
local function drawCenteredLogo()
    ...
    -- Вычисляем стартовую x для центрирования (максимальная длина строки ~ 66 символов)
    local maxWidth = 66
    ...
    for i, line in ipairs(exchangerLines) do
        gpu.set(startX, startY + 6 + i - 1, line) -- отступ после 6 строк DRAGON
    end
end
...
local function main()
    gpu.fill(1, 1, w, h, " ")
    drawCenteredLogo()          -- рисуем логотип сверху по центру
    if updInfo() then
        center(h - 12, "Для обмена встаньте на PIM и не сходите до окончания обмена", 0xffffff)
    end
    center(h - 11, "Обновлю доступные руды и связь с МЭ как только наступите", 0x505050)
